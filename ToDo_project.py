...
class User:
    def __init__(self,file_name: str, user_name: str, password: int):
        ...
        while (True):
            ...
            for name in name_list:
                ...
                if (name == self.user_name):
                    ...
                    self.user_name = input('имя пользователя существует, введите другое')

            break
        '''проверка существующих  имен'''

        self.cur.execute(f"""INSERT INTO users (id, name, password) VALUES ({new_id}, '{self.user_name}', '{self.password}')""")
...
# Danil = User('first_list', 'Danil', 123)
# Danil.check_id('Danil')


# FirstList.create('задача 1', 'тест', 'сегодня', 'выдана', 'важная', )
